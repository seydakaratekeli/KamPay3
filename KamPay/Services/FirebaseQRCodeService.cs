...
namespace KamPay.Services
{
    ...
    public class FirebaseQRCodeService : IQRCodeService
    {
        ...
        private readonly KamPay.Services.Shared.TransactionCompletionHelper _transactionCompletionHelper;
        private const string QRCodesCollection = "delivery_qrcodes";
        
        // ğŸ“Œ GÃ¼venlik sabitleri
        private const int MaxExtensionMinutes = 30;
        private const int ExtendTimeThresholdMinutes = 15;
        
        // âœ… EKLEME: QR kod tarama iÃ§in SemaphoreSlim (race condition Ã¶nleme)
        ...
        public FirebaseQRCodeService(
            ...
            KamPay.Services.Shared.TransactionCompletionHelper transactionCompletionHelper)
        {
            _firebaseClient = firebaseClient ?? throw new ArgumentNullException(nameof(firebaseClient));
            _userProfileService = userProfileService ?? throw new ArgumentNullException(nameof(userProfileService));
            _storageService = storageService ?? throw new ArgumentNullException(nameof(storageService));
            _transactionCompletionHelper = transactionCompletionHelper ?? throw new ArgumentNullException(nameof(transactionCompletionHelper));
            
            KamPay.Helpers.AppLogger.DebugLog("âœ… FirebaseQRCodeService oluÅŸturuldu (DI ile)");
        }
        ...
        public async Task<ServiceResult<bool>> CompleteDeliveryAsync(string qrCodeId)
        {
            try
            {
                ...
                if (transaction != null)
                {
                    await _transactionCompletionHelper.TryCompleteTransactionIfAllQRsUsedAsync(transaction.TransactionId);
                }
        ...
        {
            // âœ… EKLEME: Mutex kilidi (race condition Ã¶nleme)
            ...
            try
            {
                ...
                // 3. âœ… EKLEME: Double-check - Zaten kullanÄ±lmÄ±ÅŸ mÄ±? (race condition double-check)
                if (delivery.IsUsed)
                {
                ...
                // 7. FotoÄŸraf kontrolÃ¼ ()
                // Not: Location ve status gÃ¼ncellemesi kasÄ±tlÄ± - kullanÄ±cÄ± tÃ¼m kontrolleri geÃ§ti,
                // fotoÄŸraf yÃ¼kleme bekleniyor. Bu bir hata deÄŸil, workflow'un bir parÃ§asÄ±.
                if (delivery.PhotoRequired && string.IsNullOrEmpty(delivery.DeliveryPhotoUrl))
                {
                    ...
                    await deliveryNode.PutAsync(delivery);
                    return ServiceResult<bool>.FailureResult("LÃ¼tfen teslimat fotoÄŸrafÄ± yÃ¼kleyin", "PHOTO_REQUIRED");
                }
            ...
            {
                // âœ… EKLEME: Mutex kilidini serbest bÄ±rak
                _qrScanLock.Release();
            }
        }
        ...
        private string GeneratePin() => KamPay.Services.Shared.OtpGenerator.GenerateSecureOtp();
