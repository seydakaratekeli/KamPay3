                    ...
                    try
                    {
                        ...
                        if (apiResponse.IsSuccessStatusCode)
                        {
                            ...
                            if (responseData != null && !string.IsNullOrEmpty(responseData.Token))
                            {
                                ...
                                KamPay.Helpers.AppLogger.DebugLog($"✅ KamPay API JWT başarıyla alındı ve kaydedildi. (Token uzunluğu: {responseData.Token.Length})");
                            }
                            else
                            {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(tokenExpiryStr) && DateTime.TryParse(tokenExpiryStr, out var tokenExpiry))
                {
                    if (DateTime.UtcNow >= tokenExpiry)
                    {
                        ...
                        try
                        {
                            var refreshedAuth = await _authProvider.RefreshAuthAsync(new FirebaseAuthLink(_authProvider, new FirebaseAuth
                            {
                            ...
                            try
                            {
                                ...
                                if (apiResponse.IsSuccessStatusCode)
                                {
                                    ...
                                    {
                                        await Microsoft.Maui.Storage.SecureStorage.SetAsync("KAMPAY_API_JWT", responseData.Token);
                                        KamPay.Helpers.AppLogger.DebugLog($"✅ KamPay API JWT başarıyla yenilendi. (Token uzunluğu: {responseData.Token.Length})");
                                    }
                                }
                            }
                            catch (Exception apiEx)
                ...
                if (string.IsNullOrEmpty(apiToken))
                {
                    try
                    {
                        ...
                        if (apiResponse.IsSuccessStatusCode)
                        {
                            ...
                            {
                                await Microsoft.Maui.Storage.SecureStorage.SetAsync("KAMPAY_API_JWT", responseData.Token);
                                KamPay.Helpers.AppLogger.DebugLog($"✅ TryAutoLogin: KamPay API JWT yeniden alındı. (Token uzunluğu: {responseData.Token.Length})");
                            }
                        }
                        else
                        {
