...
/// <summary>
/// ? Havale/EFT simülasyon provider'ı
/// Banka referans kodu oluşturur
/// </summary>
        ...
        string currency = "TRY")
    {
        try
        {
            ...
            KamPay.Helpers.AppLogger.DebugLog(
                $"? {ProviderName}: Referans oluşturuldu - {payment.BankReference}");
            
            return ServiceResult<PaymentDto>.SuccessResult(payment, 
                $"Havale bilgileri oluşturuldu. Referans: {payment.BankReference}");
        }
        catch (Exception ex)
        {
            return ServiceResult<PaymentDto>.FailureResult("Ödeme başlatılamadı", ex.Message);
        }
    }
